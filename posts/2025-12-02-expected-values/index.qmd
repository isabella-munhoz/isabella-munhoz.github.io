...
Claro que a resposta certa seria **SIM**, mas, para não dizerem por aí que estou favorecendo meu próprio signo, vamos supor outra realidade: entre os 1 milhão de habitantes, 200 mil concordam que Gêmeos é o melhor signo, enquanto 800 mil discordam (eu sei... as pessoas têm alguma resistência com geminianos... completamente injustificada, aliás).
...
*Será que Ana vai acabar ganhando dinheiro com esse jogo?*
...
Como geramos a população desse mundo fictício, vamos ver se isso acontece na prática?

Abaixo, amostramos 10 pessoas da nossa população (ou seja, as 10 pessoas aleatórias que Ana e Bia encontrariam na rua) e calculamos quanto Ana teria ganhado nesse cenário.
...
Como estamos em um ambiente de imaginação (e temos a população interia desse mundo), podemos extrair inúmeas e inúmeras amostras para ver qual seria o cenário mais comum. Para isso, retiramos 1000 amostras da população e, para cada amostra, calculamos qual o retorno financeiro aquela amostra geraria para Ana. Na tabela abaixo, temos o exemplo de 10 dessas 1000 amostras.
...
Podemos notar nesses 10 exemplos que em alguns cenários Ana ganha mais dinheiro do que em outros cenários. Como olhar dados em tabelas não nos ajudam a tirar grandes conclusões, o gráfico abaixo nos ajuda a visualizar o balanço financeiro de todas as 1000 amostras que extraímos acima.
...
Quando comecei a estudar valor esperado, encontrei diversas definições que, embora se complementassem, às vezes me deixavam confusa. Nesta seção, minha proposta é organizar essas ideias e mostrar como elas se conectam, para que o conceito de valor esperado fique mais intuitivo, independentemente da forma pela qual você o enxergue.

## Cálculo a partir de experimentos 
...
## Cálculo a partir da fórmula teórica
...
## Cálculo a partir da distribuição populacional

O valor esperado também pode ser entendido como a **média populacional que obteríamos se tivéssemos acesso a todos os dados da população**. Como normalmente não dispomos de todas as observações possíveis, essa média é tratada como um valor teórico, definido pelas probabilidades da variável aleatória em estudo. No nosso exemplo, porém, temos a população completa, o que nos permite calcular diretamente essa média, como mostrado abaixo.
...
population |> 
  ...
  mk_tbl(
    table_title = "Média populacional"
  )

```
<br>
Considerando que cada resposta "sim" gera um retorno de +10 reais e cada resposta "não" gera um retorno de –1 real, então a média calculada sobre toda a população resulta em um valor de R\$ 1.20. Esse valor coincide exatamente com o que obtivemos pela fórmula do valor esperado: em média, **cada pessoa** entrevistada gera um ganho esperado de R\$ 1.20 (logo, 10 pessoas geram um ganho esperado de R\$ 12).

## Um resumo de todas as definições
...
- **Uma média empírica**, interpretada como o resultado médio que esperamos obter ao repetir um experimento aleatório muitas e muitas vezes sob as mesmas condições.

- **Uma média teórica**, calculada pela fórmula $\mathbb{E}[X] = \sum_{i=1}^n x_i p_i$, que combina cada resultado possível com sua probabilidade.

- **A média populacional**, que obteríamos caso tivéssemos acesso a todas as observações geradas pela variável aleatória.

O valor esperado é, portanto, a medida teórica que descreve o valor médio de uma variável aleatória, a média "verdadeira" (populacional) de sua distribuição, e funciona como o parâmetro central ao qual as médias observadas nas amostras tendem a se aproximar conforme coletamos mais e mais dados.

# Valor esperado em diferentes distribuições

Conforme já mencionado, variáveis aleatórias são valores gerados por algum processo incerto, como a altura das pessoas, o resultado de um experimento ou comportamentos que variam de indivíduo para indivíduo. No exemplo da aposta entre Ana e Bia, o experimento aleatório consiste em entrevistar 10 pessoas e registrar a resposta de cada uma à pergunta *"Gêmeos é o melhor signo do zodíaco, né?"*. Ao associarmos as respostas "sim" ou "não" aos retornos financeiros correspondentes, obtemos nossa variável aleatória.

Apesar de incertas, variáveis aleatórias não se comportam de qualquer maneira: elas seguem o que chamamos de **distribuição**. Uma distribuição descreve como os possíveis valores de uma variável aleatória se espalham e com que frequência cada valor tende a aparecer. Em outras palavras, ela é a "história" dos dados: mostra quais resultados são comuns, quais são raros e qual é o comportamento geral daquela variável na população.

No exemplo que estamos usando, ao entrevistar 10 pessoas, calculamos o retorno financeiro daquele grupo. Esse retorno segue uma distribuição chamada **Binomial**, que descreve situações em que repetimos o mesmo experimento diversas vezes (fazer a mesma pergunta para várias pessoas) e cada tentativa só pode resultar em duas saídas possíveis, como "sim" ou "não". A distribuição Binomial nos diz quantas vezes um determinado resultado tende a aparecer ao longo dessas repetições, assumindo que cada pessoa responde de forma independente e que a probabilidade de responder "sim" é sempre a mesma.

Além da Binomial, existem várias outras distribuições muito usadas na prática, como a Normal, a Gamma, a Poisson, a Beta, e por aí vai... Cada uma descreve comportamentos e padrões distintos que aparecem em diferentes tipos de dados. 

No gráfico abaixo, simulamos populações que seguem algumas dessas distribuições para observar visualmente como elas se apresentam.
...
distributions |> 
  ...
  labs(
    ...
    y = "quantidade",
    title = "Exemplo de populações que seguem diferentes distribuições"
  )

```


O objetivo deste post não é aprofundar no estudo das distribuições, mas sim discutir o valor esperado. E aqui vem o ponto-chave que gostaria de trazer nessa seção: **o conceito de valor esperado é o mesmo para qualquer distribuição**. Independentemente da forma da distribuição populacional da variável aleatória, **o valor esperado é sempre a média populacional**.

Abaixo, mostramos para cada distribuição simulada anteriormente o cálculo da média populacional (ou seja, a média considerando todos os valores gerados naquela população simulada).
...
distributions |> 
  ...
  mk_tbl(
    table_title = "Média populacional de diferentes distribuições"
  )

```

<br>
Em seguida, aplicamos, para cada uma dessas populações, o cálculo do valor esperado pelo método empírico descrito na seção anterior e podemos observar que o valor obtido empiricamente é praticamente igual à média populacional.
...
distributions_samples |> 
  ...
  mk_tbl(
    table_title = "Valor esperado de diferentes distribuições"
  )

```

<br>
*Importante: para cada distribuição, também poderíamos calcular o valor esperado usando a fórmula teórica ou diretamente a partir dos parâmetros da distribuição. Para manter o texto mais simples, ficamos apenas com o cálculo empírico.*

Saber que o valor esperado é a média teórica de uma distribuição é importante porque ele representa o ponto em torno do qual a variável aleatória "se organiza": o seu verdadeiro "centro de gravidade". Todavia, na prática, quase nunca temos acesso à distribuição completa (ou seja, aos dados populacionais). O que temos são amostras da população real. Por isso, o nosso trabalho costuma ser estimar a média populacional a partir da média amostral. Quando entendemos que o valor esperado é justamente essa **média teórica** que estamos tentando aproximar com os dados observados, conseguimos interpretar melhor o que nossas estimativas significam, avaliar o quanto elas estão próximas do "centro" real da variável e tomar decisões mais informadas mesmo diante da incerteza. Compreender o valor esperado, portanto, é entender o ponto de equilíbrio da variável, seja olhando para toda a distribuição, seja olhando para os dados que conseguimos observar.

# A melhor aposta

Voltando ao tema central deste post, chegamos à conclusão de que o resultado da aposta entre Ana e Bia depende, no fim das contas, da SORTE. Porém, com um pouco de estatística, conseguimos ir além do palpite intuitivo. A partir da definição de valor esperado, descobrimos algo importante: a estrutura da aposta favorece Ana, já que o resultado financeiro mais provável gira em torno de 12 reais de ganho.

Contudo, uma outra pergunta surge a partir do que estudamos: se Ana decidisse mudar o valor pago por cada pessoa que responde "sim", haveria algum ponto em que a aposta se tornaria desfavorável para ela?

É exatamente aqui que o valor esperado mostra sua força. Ele permite que Ana explore diferentes cenários antes mesmo de fazer a aposta. Basta variar o valor pago por cada resposta "sim" e calcular, para cada possibilidade, se o balanço financeiro esperado é positivo ou negativo. 

No código abaixo, calculamos o valor esperado da aposta considerando que o valor da aposta para as respostas "sim" dos entrevistados poderia variar entre R\$ 1 e R\$ 20. Pelo gráfico a seguir, observamos que, se o valor apostado por resposta "sim" fosse menor que R\$ 5, Ana tenderia a perder dinheiro!
...
O mais interessante é perceber que essa lógica não vale só para a aposta da Ana, ela vale para praticamente qualquer situação incerta da vida: investimentos, decisões de produto, estratégias de negócio, experimentos, pesquisas, escolhas do dia a dia. **O valor esperado nos dá uma forma racional de enxergar onde está o ponto de equilíbrio de um processo incerto.**

E, no fundo, é isso que torna a estatística tão fascinante: assim como o tarô e a astrologia, ela tenta lançar luz sobre o desconhecido, e, a partir de ferramentas que nos permitem quantificar incertezas, nos ajuda a comparar cenários e entender, com mais clareza, o que podemos esperar do futuro. Estatística não elimina o acaso, mas nos ajuda a navegar por ele com muito mais sabedoria.
